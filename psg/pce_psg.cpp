...
int PCE_PSG::StateAction(StateMem *sm, int load, int data_only)
{
 ...
 for(int ch = 0; ch < 6; ch++)
 {
  ...
  tmpstr[3] = '0' + ch;
  ret &= MDFNSS_StateAction(sm, load, data_only, CH_Statem_regs, tmpstr);
 }

 SFORMAT PSG_Statem_regs[] =
 {
  SFVAR(select),
  SFVAR(globalbalance),
  SFVAR(lfofreq),
 ...
 ret &= MDFNSS_StateAction(sm, load, data_only, PSG_Statem_regs, "PSG");
